...
func Add(c *gin.Context) {
	...
	if result.Err != nil {
		c.JSON(http.StatusUnauthorized, jpnic.Result{Status: false, Error: result.Err.Error()})
		return
	}

	if err := check(input); err != nil {
		c.JSON(http.StatusBadRequest, jpnic.Result{Status: false, Error: err.Error()})
		return
	}

	// check authority
	if result.User.Level > 1 {
		c.JSON(http.StatusUnauthorized, jpnic.Result{Status: false, Error: "You don't have authority this operation"})
		return
	}
...
func Delete(c *gin.Context) {
	...
	if id <= 0 {
		c.JSON(http.StatusBadRequest, jpnic.Result{Status: false, Error: "wrong id"})
		return
	}
...
func Get(c *gin.Context) {
	...
	c.JSON(http.StatusOK, jpnic.Result{Status: true, Jpnic: data})
}
