...
func AddByAdmin(c *gin.Context) {
	...
	if _, err = dbNTTTemplate.Create(&input); err != nil {
...
func DeleteByAdmin(c *gin.Context) {
	...
	if err := dbNTTTemplate.Delete(&core.NTTTemplate{Model: gorm.Model{ID: uint(id)}}); err != nil {
		c.JSON(http.StatusInternalServerError, common.Error{Error: err.Error()})
		return
	}
	c.JSON(http.StatusOK, ntt.Result{})
...
func UpdateByAdmin(c *gin.Context) {
	...
	tmp := dbNTTTemplate.Get(service.ID, &core.NTTTemplate{Model: gorm.Model{ID: input.ID}})
	if tmp.Err != nil {
		c.JSON(http.StatusInternalServerError, common.Error{Error: tmp.Err.Error()})
		return
	}

	if err = dbNTTTemplate.Update(service.UpdateAll, update(input, tmp.NTTs[0])); err != nil {
		c.JSON(http.StatusInternalServerError, common.Error{Error: err.Error()})
		return
	}
	c.JSON(http.StatusOK, ntt.Result{})
...
func GetByAdmin(c *gin.Context) {
	...
	result := dbNTTTemplate.Get(ntt.ID, &core.NTTTemplate{Model: gorm.Model{ID: uint(id)}})
	if result.Err != nil {
...
func GetAllByAdmin(c *gin.Context) {
	...
	if result := dbNTTTemplate.GetAll(); result.Err != nil {
		c.JSON(http.StatusInternalServerError, common.Error{Error: result.Err.Error()})
	} else {
		c.JSON(http.StatusOK, ntt.Result{NTTs: result.NTTs})
	}
